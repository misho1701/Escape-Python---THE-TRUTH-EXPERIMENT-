...
A text adventure game built in Python. You wake up in a prison cell with no memory
of who you are or how you got there. Your only goal: escape. But the deeper you go,
the more you realise the facility has a darker secret — and so do you.
...
escape_python/
│
├── main.py          — Entry point. Menu, game loop, ending display.
...
├── endings.py       — Ending definitions. Good, bad, secret.
...
| Room | Description | Puzzle |
|---|---|---|
| **Cell** | Starting room. Cold, damp. Door locked. | Number riddle — decode clues from the Note |
| **Hall** | Flickering corridor. Guard patrols here. NPC dialogue. | None |
| **Ventilation Shaft** | Cramped ducts above the Hall. Hidden page inside. | Clock riddle — which position at midnight |
| **Server Room** | Blue-lit server racks. Console terminal. | Employee ID authentication |
| **Laboratory** | Broken monitors, active terminal. Story revelation here. | Terminal activation — power it up |
| **Exit** | Three-lock door. Point of no return. | Card reader + power conduit + logic riddle |
...
| **JournalPage** | Ventilation Shaft | Reveals the server room ID (`7b3`) through narrative |
| **KeycardFragment** | Server Room | Repairs the AccessCard — required for Exit Lock 1 |
| **DataChip** | Server Room | Inserts into the lab terminal to reveal the secret ending lore |
...
The final door requires three steps in order:

1. **Lock 1 — Card Reader** — `use AccessCard` (must have KeycardFragment in inventory to repair it first)
2. **Lock 2 — Power Conduit** — `use Wire` or `use Battery` (requires both in inventory, clears the magnetic seal)
3. **Lock 3 — Logic Gate** — `puzzle` then `solve echo` (only available after locks 1 and 2)

Use `status` while in the Exit room to check progress.
...
| Ending | Condition |
|---|---|
| **Good** | Solve all three exit locks and escape |
| **Secret** | Escape after discovering the truth via the DataChip and Guard dialogue |
| **Bad** | *(reserved — currently unused, available for future trigger)* |
...
| **Truth Seeker** | Learn the truth about Project ECHO |
...
use <item>          Use an item (context-aware — effects depend on location)
...
Progress is saved to `save.json` in the project root. The file stores:
- Current room
- Inventory contents
- All story flags
